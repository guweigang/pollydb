...
fn query_sample_filter(column_name string, plugin_name string, selector string, value_type ColumnType, op QueryFilterOp) QueryFilter {
	...
	if plugin_name.len > 0 || selector.len > 0 {
		return match op {
			...
			.between { QueryFilter.field_between(column_name, plugin_name, selector, first,
					second) }
		}
	}
	return match op {
		...
		match filter.op {
			.before, .after, .between, .prefix, .eq {
				order_by = QueryOrder{
					...
					direction:   if filter.op in [.before, .after, .between, .prefix, .eq] {
						.desc
					} else {
						.asc
					}
	...
	for op in query_supported_ops_for_type(value_type) {
		...
		shapes << QueryFilterShapeCapability{
			...
			supports_top_n:        best_score >= 0 && plugin_name.len == 0 && selector.len == 0
				&& op in [.eq, .before, .after, .between, .prefix]
			sample_explain:        query_sample_plan_explain(spec, projectors, table_name,
				filter)
		}
	}
	return shapes
...
fn query_sample_general_fts_explain(spec TypedTableSpec, index SchemaIndexDef, kind FtsQueryKind) QuerySamplePlanExplain {
	...
	return QuerySamplePlanExplain{
		...
		warnings:                    []string{}
		notes:                       [
			'Query can be issued via QueryRequest.general_fts using the `${index.name}` index.',
	...
	for index in spec.indexes {
		...
		field_selector_entries[key] = QueryFieldSelectorCapability{
			...
			filter_shapes:   query_filter_shapes_for_target(spec, database.projectors,
				table_name, index.column, meta.plugin_name, meta.selector, meta.value_type,
