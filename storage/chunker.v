...
pub fn (cfg ChunkConfig) with_detailed_timings(enabled bool) ChunkConfig {
	return ChunkConfig{
		...
		small_patch_limit:               cfg.small_patch_limit
		...
		enable_write_diff:               cfg.enable_write_diff
	}
}
...
pub fn (cfg ChunkConfig) with_small_patch_limit(limit int) ChunkConfig {
	return ChunkConfig{
		min_size:                        cfg.min_size
		max_size:                        cfg.max_size
		mask:                            cfg.mask
		detailed_timings:                cfg.detailed_timings
		small_patch_limit:               limit
		enable_partitioned_rebuild:      cfg.enable_partitioned_rebuild
		force_partitioned_rebuild:       cfg.force_partitioned_rebuild
		enable_split_backed_working_set: cfg.enable_split_backed_working_set
		enable_write_diff:               cfg.enable_write_diff
...
pub fn (cfg ChunkConfig) with_write_diff(enabled bool) ChunkConfig {
	return ChunkConfig{
		min_size:                        cfg.min_size
		max_size:                        cfg.max_size
		mask:                            cfg.mask
...
pub fn (cfg ChunkConfig) chunk_bytes(data []u8) ![]Chunk {
	...
	unsafe {
		base := &u8(data.data)
		for start < data.len {
			boundary := fast_cdc_core(base + start, data.len - start, cfg.min_size, cfg.max_size,
				cfg.mask)
...
pub fn (mgr BufferManager) ingest_to_store(cfg ChunkConfig, ingest_cfg ChunkIngestConfig, mut store ChunkStore) !ChunkIngestResult {
	...
	if normalized.collect_chunks {
		chunk_cids := mgr.chunk_file_cids_to_store_with_workers(cfg, mut store,
...
pub fn hash_chunks_parallel(data []u8, chunks []Chunk, worker_count int) []ChunkCid {
	...
	for start in 0 .. actual_workers {
		...
		batch_end := if batch_start + batch_size < chunks.len {
	...
	unsafe {
		...
		for start < data.len {
			...
			if batch.len >= batch_size {
				batches << []ChunkCid{}
				wg.add(1)
				spawn hash_chunk_batch_into_slot(data, batch.clone(), slot, voidptr(&batches), mut
					wg)
				slot++
