...
fn write_checkpoint_journal(root_dir string, node_records []u8, commit_records []u8) ! {
	...
	payload << commit_records
	mut data := []u8{}
	data << repository_checkpoint_journal_magic.bytes()
	repository_append_u32(mut data, repository_checkpoint_journal_version)
	data << payload
...
fn replay_checkpoint_journal(root_dir string) ! {
	...
	mut commit_store := PersistentCommitStore.open_high_throughput(repository_commits_path(root_dir))!
	defer {
		commit_store.close()
	}
	mut cursor := 0
	...
	for cursor < data.len {
		...
		version := u32(data[cursor + 4]) | (u32(data[cursor + 5]) << 8) | (u32(data[cursor + 6]) << 16) | (u32(data[cursor + 7]) << 24)
...
pub fn (repo Repository) persist(path string) ! {
	os.write_file(path, repo.data().bytestr())!
}
...
pub fn (repo Repository) branch(name string) !Branch {
	commit_cid := repo.branches[name] or {
...
pub fn (mut persistent PersistentRepository) compare_and_swap_branch_head(name string, old_commit_cid string, new_commit_cid string) !Branch {
	...
	swapped := backend.compare_and_swap_branch_head(name, old_commit_cid, new_commit_cid)!
	if !swapped {
		return error('branch head changed during compare-and-swap: ${name}')
	}
	persistent.repo = Repository.open(persistent.path)!
	return persistent.repo.branch(name)
}

pub fn (mut persistent PersistentRepository) commit_to_branch(branch_name string, tree Tree, meta CommitMeta) !BranchUpdate {
	update := persistent.repo.commit_to_branch(branch_name, tree, meta, mut persistent.node_store, mut persistent.commit_store)!
	persistent.meta_dirty = true
	persistent.persist()!
	return update
}
...
pub fn (mut persistent PersistentRepository) tree_at_branch(branch_name string) !Tree {
	return persistent.repo.tree_at_branch(branch_name, mut persistent.node_store, mut persistent.commit_store)
}
...
pub fn (mut repo Repository) apply_mutations_to_branch(branch_name string, mutations []Mutation, cfg ChunkConfig, meta CommitMeta, mut node_store NodeStore, mut commit_store CommitStore) !BranchMutationResult {
	...
	update := repo.commit_to_branch(branch_name, tree_update.tree, meta, mut node_store, mut commit_store)!
...
pub fn (mut repo Repository) apply_write_set_to_branch(branch_name string, specs []TableSpec, write_set WriteSet, cfg ChunkConfig, meta CommitMeta, mut node_store NodeStore, mut commit_store CommitStore) !BranchTransactionResult {
	...
	update := repo.commit_to_branch(branch_name, transaction_update.tx.current_tree(), meta, mut node_store, mut commit_store)!
...
pub fn (mut repo Repository) apply_typed_write_set_to_branch(branch_name string, specs []TypedTableSpec, write_set TypedWriteSet, cfg ChunkConfig, meta CommitMeta, mut node_store NodeStore, mut commit_store CommitStore) !BranchTypedTransactionResult {
	...
	snapshot := Snapshot.new(transaction_update.tx.current_tree(), repo.parent_cids_for_branch(branch_name), meta)
	sw.restart()
	snapshot.persist(mut node_store, mut commit_store)!
...
pub fn (mut repo Repository) commit_working_set(mut set WorkingSet, meta CommitMeta, mut node_store NodeStore, mut commit_store CommitStore) !BranchTransactionResult {
	...
	update := repo.commit_to_branch(set.branch_name, set.transaction().current_tree(), meta, mut node_store, mut commit_store)!
...
pub fn (repo Repository) merge_branch_into_working_set(mut set WorkingSet, theirs_branch string, resolutions []ConflictResolution, cfg ChunkConfig, mut node_store NodeStore, mut commit_store CommitStore) !WorkingSetMergeResult {
	base_commit := repo.merge_base_commit(set.base_commit_cid, (repo.branch(theirs_branch)!).commit_cid, mut commit_store)!
	...
	merge_result := three_way_merge(base_commit, ours_commit, theirs_commit, base_tree, ours_tree, theirs_tree, cfg)!
...
pub fn (repo Repository) typed_merge_branch_into_working_set(mut set TypedWorkingSet, theirs_branch string, resolutions []ConflictResolution, cfg ChunkConfig, mut node_store NodeStore, mut commit_store CommitStore) !WorkingSetMergeResult {
	...
	normalized_tree, reindex_timings := rebuild_typed_indexes_for_changed_rows(resolution.tree, set.specs, changed_rows, cfg)!
...
fn changed_row_keys_for_subtree(base_tree Tree, source_tree Tree, base_node Node, source_node Node, row_prefixes []string) ![]string {
	...
	for {
		...
		key := if base_has && (!source_has || compare_key_bytes(base_items[base_idx].key, source_items[source_idx].key) <= 0) {
			base_items[base_idx].key
		} else {
			source_items[source_idx].key
		}
		base_val, base_present := merge_item_value_for_key(if base_has { base_items[base_idx] } else { KVPair{} }, base_has, key)
		source_val, source_present := merge_item_value_for_key(if source_has { source_items[source_idx] } else { KVPair{} }, source_has, key)
		if base_present {
			base_idx++
		}
		if source_present {
...
pub fn (mut repo Repository) merge_branch_into(ours_branch string, theirs_branch string, resolutions []ConflictResolution, cfg ChunkConfig, meta CommitMeta, mut node_store NodeStore, mut commit_store CommitStore) !BranchUpdate {
	result := repo.merge_branches(ours_branch, theirs_branch, cfg, mut node_store, mut commit_store)!
	resolution := result.resolve_conflicts(resolutions, cfg)!
	return repo.commit_to_branch(ours_branch, resolution.tree, meta, mut node_store, mut commit_store)!
}
...
fn three_way_merge(base_commit Commit, ours_commit Commit, theirs_commit Commit, base_tree Tree, ours_tree Tree, theirs_tree Tree, cfg ChunkConfig) !MergeResult {
	...
	merged_result := merge_subtree_refs(mut builder, base_tree, ours_tree, theirs_tree, base_root, ours_root,
		theirs_root)!
...
fn merge_subtree_refs(mut builder TreeBuilder, base_tree Tree, ours_tree Tree, theirs_tree Tree, base_node Node, ours_node Node, theirs_node Node) !MergeRefResult {
	...
	if base_node.kind == .internal && ours_node.kind == .internal && theirs_node.kind == .internal {
		...
		suffix_len := common_child_suffix_len(base_children, ours_children, theirs_children, prefix_len)
		...
		if base_mid_len >= 0 && base_mid_len == ours_mid_len && base_mid_len == theirs_mid_len {
			for idx in 0 .. base_mid_len {
				...
				child_result := merge_subtree_refs(mut builder, base_tree, ours_tree, theirs_tree, base_child,
					ours_child,
					theirs_child)!
			...
			theirs_mid := subtree_items_range(theirs_tree, theirs_children, prefix_len, theirs_mid_end)!
...
fn merge_item_slices(base_items []KVPair, ours_items []KVPair, theirs_items []KVPair) !MergeSliceResult {
	...
	for {
		...
		key := merge_min_key(
			if base_has { base_items[base_idx] } else { KVPair{} }, base_has,
			if ours_has { ours_items[ours_idx] } else { KVPair{} }, ours_has,
			if theirs_has { theirs_items[theirs_idx] } else { KVPair{} }, theirs_has,
		)
		base_val, base_present := merge_item_value_for_key(
			if base_has { base_items[base_idx] } else { KVPair{} }, base_has, key)
		ours_val, ours_present := merge_item_value_for_key(
			if ours_has { ours_items[ours_idx] } else { KVPair{} }, ours_has, key)
		theirs_val, theirs_present := merge_item_value_for_key(
			if theirs_has { theirs_items[theirs_idx] } else { KVPair{} }, theirs_has, key)
		if base_present {
			base_idx++
		}
		if ours_present {
		...
		if ours_same_as_base && theirs_same_as_base {
			if base_present {
				merged << KVPair{key: key.clone(), value: base_val}
			}
			continue
		}
		if ours_same_as_base {
			if theirs_present {
				merged << KVPair{key: key.clone(), value: theirs_val}
			}
			changed_keys << key.bytestr()
			continue
		}
		if theirs_same_as_base {
			if ours_present {
				merged << KVPair{key: key.clone(), value: ours_val}
			}
			changed_keys << key.bytestr()
			continue
		}
