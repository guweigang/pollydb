...
fn (mut store ChunkStore) rebuild_index_from_mmap() ! {
	...
	unsafe {
		mapped := C.mmap(voidptr(0), usize(store.write_offset), C.PROT_READ, C.MAP_SHARED, store.file.fd, 0)
		if mapped == voidptr(-1) {
		...
		for offset < store.write_offset {
			...
			cid_len := int(*(header)) | (int(*(header + 1)) << 8) | (int(*(header + 2)) << 16) | (int(*(header + 3)) << 24)
			data_len := int(*(header + 4)) | (int(*(header + 5)) << 8) | (int(*(header + 6)) << 16) | (int(*(header + 7)) << 24)
