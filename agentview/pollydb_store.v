...
pub fn (store PollyDbStore) ensure_search_indexes() !bool {
	stats := store.ensure_search_indexes_with_progress_and_config(no_search_index_progress, storage.ChunkConfig.default())!
...
pub fn (store PollyDbStore) ensure_search_indexes_with_progress_and_config(reporter SearchIndexProgressReporter, cfg storage.ChunkConfig) !SearchIndexStats {
	...
	ingest_states := load_existing_ingest_states_by_path(mut db) or { map[string]IngestState{} }
	entry_ingest_states := load_all_entry_ingest_states(mut db) or { map[string]EntryIngestState{} }
	entry_search_states := load_all_entry_search_states(mut db) or { map[string]EntrySearchState{} }
	...
	if target_entry_ids.len > 0 || stale_entry_search_ids.len > 0 {
		write_entry_search_states(mut db, entry_ingest_states, target_entry_ids, stale_entry_search_ids, effective_cfg)!
	}
	...
	stats := SearchIndexStats{
		...
		changed: changed || force_rebuild || backfill.rows_scanned > 0 || stale_entry_search_ids.len > 0
	}
	reporter(SearchIndexProgress{
		phase: 'done'
...
pub fn (store PollyDbStore) sync_codex_with_progress_and_config(codex_root string, reporter SyncProgressReporter, cfg storage.ChunkConfig) !SyncStats {
	...
	for path in paths {
		...
		candidate_id := if state.session_id.len > 0 { state.session_id } else { session_id_from_path(path) }
		...
		source_changed := !(state.path.len > 0 && state.source_mtime_unix == fingerprint.source_mtime_unix
			&& state.source_size_bytes == fingerprint.source_size_bytes)
		...
		for entry in transcript.entries {
			...
			entry_state_rows[entry_id] = build_entry_ingest_state_row(entry_id, summary.id, entry_hash)
			entry_count++
		}
		...
		if use_split_group_commit {
			...
			if stale_entry_ids.len > 0 {
				...
				checkpoint_ms += delete_result.group_commit.checkpoint_ms
				flush_ms += delete_result.group_commit.flush_ms
				entry_state_delete_result := session.delete_rows(mut db, 'entry_ingest_state', stale_entry_ids, cfg,
			...
			if entry_state_rows.len > 0 {
				entry_state_result := session.put_rows(mut db, 'entry_ingest_state', entry_state_rows, cfg,
					sync_meta('sync entry ingest state ${summary.id}'))!
				tx_ms += entry_state_result.group_commit.transaction_ms
...
pub fn (mut store_session BrowserStoreSession) list_sessions_page_explained(request SessionListRequest) !SessionListExecution {
	mut total_sw := time.new_stopwatch()
	return list_sessions_page_in_session(mut store_session.db, store_session.session, request,
		storage.PersistentDatabaseOpenTimings{}, 0, mut total_sw)
}
...
pub fn (store PollyDbStore) list_sessions_page_explained(request SessionListRequest) !SessionListExecution {
	...
	return list_sessions_page_in_session(mut db, session, request, open_result.timings, session_ms,
		mut total_sw)!
}
...
fn list_sessions_page_in_session(mut db storage.PersistentDatabase, session storage.DatabaseSession, request SessionListRequest, open_timings storage.PersistentDatabaseOpenTimings, session_ms i64, mut total_sw time.StopWatch) !SessionListExecution {
	if request.query.len == 0 && request.cwd_prefix.len == 0 && request.source.len == 0
		...
		fetch_limit := max_int(request.offset + max_int(request.limit, 0), max_int(request.limit, 0))
		profile := session.query_page_profiled(mut db, storage.QueryRequest{
			table_name: 'sessions'
			order_by: storage.QueryOrder{
		...
		lower_bound_total := if page.cursor.has_more { request.offset + out.len + 1 } else { out.len }
		return SessionListExecution{
			result: SessionListResult{
				total: lower_bound_total
				sessions: out[start..end].clone()
			}
			explain: explain_session_list_path(session.table_spec('sessions') or { storage.TypedTableSpec{} }, request)
	...
	if request.query.len > 0 || request.cwd_prefix.len > 0 || request.source.len > 0 || !request.include_archived {
...
fn explain_session_list_path(spec storage.TypedTableSpec, request SessionListRequest) QueryPathExplain {
	if request.query.len == 0 && request.cwd_prefix.len == 0 && request.source.len == 0
		...
		return QueryPathExplain{
			...
			notes: ['session list uses query_page(order_by updated_at desc) with covering projection']
		}
	}
	if table_has_index(spec, 'updated_at_cover_idx') {
		mut notes := ['results are read from updated_at covering index and reversed to newest-first']
		if request.query.len > 0 || request.cwd_prefix.len > 0 || request.source.len > 0 || !request.include_archived {
...
fn explain_transcript_path(spec storage.TypedTableSpec, request TranscriptRequest) QueryPathExplain {
	...
	if table_has_index(spec, 'entries_session_cover_idx') {
		return QueryPathExplain{
			...
			notes: ['entries are fetched by covering session_id index, then ordered by seq in memory']
		}
	}
	if table_has_index(spec, 'entries_session_idx') {
		return QueryPathExplain{
			...
			notes: ['entries are fetched by session_id index, then ordered by seq in memory']
		}
	}
	return QueryPathExplain{
		strategy: 'table_scan'
		index_name: ''
		notes: ['entries_session_idx is missing, transcript would require full scan']
	}
}
...
fn explain_search_path(spec storage.TypedTableSpec, request SearchRequest) QueryPathExplain {
	...
	return QueryPathExplain{
		...
		index_name: ''
		notes: ['FTS indexes are missing, so browser search returns no results instead of scanning entries']
	}
}
...
pub fn (mut store_session BrowserStoreSession) load_transcript_page_explained(request TranscriptRequest) !TranscriptExecution {
	mut total_sw := time.new_stopwatch()
	return load_transcript_page_in_session(mut store_session.db, store_session.session, request,
		storage.PersistentDatabaseOpenTimings{}, 0, mut total_sw)
}
...
pub fn (store PollyDbStore) load_transcript_page_explained(request TranscriptRequest) !TranscriptExecution {
	...
	session := db.begin_session(storage.SessionOptions.for_branch(store_branch))!
	session_ms := session_sw.elapsed().milliseconds()
	return load_transcript_page_in_session(mut db, session, request, open_result.timings, session_ms,
		mut total_sw)!
}
...
fn load_transcript_page_in_session(mut db storage.PersistentDatabase, session storage.DatabaseSession, request TranscriptRequest, open_timings storage.PersistentDatabaseOpenTimings, session_ms i64, mut total_sw time.StopWatch) !TranscriptExecution {
	...
	rows := if table_has_index(session.table_spec('entries') or { storage.TypedTableSpec{} },
		...
		session.lookup_index_projected(mut db, 'entries', 'entries_session_cover_idx', request.session_id, 0,
			transcript_entry_select_columns)!
	} else {
	...
	return TranscriptExecution{
		...
		explain: explain_transcript_path(session.table_spec('entries') or { storage.TypedTableSpec{} }, request)
		open_ms: open_timings.total_ms
		open_backends_ms: open_timings.backends_ms
		open_catalog_ms: open_timings.catalog_ms
		open_engine_ms: open_timings.engine.total_ms
...
fn search_entries_in_session(mut db storage.PersistentDatabase, session storage.DatabaseSession, request SearchRequest, query string, terms []string, open_timings storage.PersistentDatabaseOpenTimings, session_ms i64, mut total_sw time.StopWatch) !SearchExecution {
	...
	mut candidate_rows := []storage.TypedSchemaRow{}
		...
		if ranked.len > 0 {
			...
			return SearchExecution{
				...
				explain: QueryPathExplain{
					...
					notes: ['no FTS hits; search fell back to substring matching inside the current session only']
				}
				open_ms: open_timings.total_ms
				open_backends_ms: open_timings.backends_ms
				open_catalog_ms: open_timings.catalog_ms
	...
	if request.session_id.len == 0 && request.preferred_session_id.len > 0 {
		...
		ranked = collect_session_local_search_hits(mut db, session, preferred_request, query, terms)!
		...
		if ranked.len > 0 {
			...
			return SearchExecution{
				...
				explain: QueryPathExplain{
					...
					notes: ['no FTS hits; search fell back to substring matching inside the currently open session']
				}
				open_ms: open_timings.total_ms
				open_backends_ms: open_timings.backends_ms
				open_catalog_ms: open_timings.catalog_ms
		...
		if ranked.len > 0 {
			...
			return SearchExecution{
				...
				explain: QueryPathExplain{
					...
					notes: ['no FTS hits; search fell back to substring matching across recent session bodies']
				}
				open_ms: open_timings.total_ms
				open_backends_ms: open_timings.backends_ms
				open_catalog_ms: open_timings.catalog_ms
		...
		if ranked.len > 0 {
			...
			return SearchExecution{
				...
				explain: QueryPathExplain{
					...
					notes: ['no FTS hits; search fell back to session title/cwd/source/path substring matching']
				}
				open_ms: open_timings.total_ms
				open_backends_ms: open_timings.backends_ms
				open_catalog_ms: open_timings.catalog_ms
	...
	return SearchExecution{
		...
		explain: if used_fts {
			...
			QueryPathExplain{
				strategy: 'no_fts_indexes'
				index_name: ''
				notes: ['FTS indexes are missing, so browser search returned no results']
			}
		}
		open_ms: open_timings.total_ms
		open_backends_ms: open_timings.backends_ms
...
fn seed_store_branch(mut db storage.PersistentDatabase, summary SessionSummary) ! {
	...
	tree = storage.rebuild_typed_indexes_for_specs(tree, [spec, ingest_state_spec()!, search_state_spec()!, entry_ingest_state_spec()!, entries_spec(false)!], cfg)!
	tree = storage.rebuild_typed_aggregates_for_specs(tree, [spec, ingest_state_spec()!, search_state_spec()!, entry_ingest_state_spec()!, entries_spec(false)!], cfg)!
...
fn fingerprint_session_entry_row(row storage.TypedRowData) string {
	mut parts := []string{}
	for column in ['id', 'session_id', 'session_title', 'seq', 'timestamp', 'role', 'kind', 'tool_name',
		'call_id', 'title', 'content_text', 'raw_type', 'phase'] {
...
fn load_existing_entry_ingest_states_by_session(mut db storage.PersistentDatabase, session_id string) !map[string]EntryIngestState {
	...
	session := db.begin_session(storage.SessionOptions.for_branch(store_branch))!
	rows := session.lookup_index(mut db, 'entry_ingest_state', 'entry_ingest_session_idx', session_id, 0) or {
		return map[string]EntryIngestState{}
	}
...
fn load_all_entry_ingest_states(mut db storage.PersistentDatabase) !map[string]EntryIngestState {
	session := db.begin_session(storage.SessionOptions.for_branch(store_branch))!
	rows := session.scan_table(mut db, 'entry_ingest_state', 0) or { return map[string]EntryIngestState{} }
	mut out := map[string]EntryIngestState{}
	for row in rows {
		state := decode_entry_ingest_state(row) or { continue }
		out[state.id] = state
...
fn load_all_entry_search_states(mut db storage.PersistentDatabase) !map[string]EntrySearchState {
	session := db.begin_session(storage.SessionOptions.for_branch(store_branch))!
	rows := session.scan_table(mut db, 'entry_search_state', 0) or { return map[string]EntrySearchState{} }
...
fn write_search_states(mut db storage.PersistentDatabase, ingest_states map[string]IngestState, session_ids []string) ! {
	...
	for _, ingest in ingest_states {
		if ingest.session_id.len == 0 || !(ingest.session_id in session_ids) || seen[ingest.session_id] {
			continue
		}
...
fn write_entry_search_states(mut db storage.PersistentDatabase, ingest_states map[string]EntryIngestState, entry_ids []string, stale_entry_ids []string, cfg storage.ChunkConfig) ! {
	...
	if entry_ids.len > 0 {
		...
		for entry_id in entry_ids {
			...
			rows[entry_id] = build_entry_search_state_row(entry_id, state.session_id, state.entry_hash)
		}
		if rows.len > 0 {
			if use_split_group_commit {
